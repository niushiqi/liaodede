...
public class CallActivity extends LinphoneGenericActivity
  ...
  private ImageView pause, hangUp, dialer, video, micro, speaker, options, addCall, transfer,
      conference, conferenceStatus, contactPicture;
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    mListener = new LinphoneCoreListenerBase() {
      @Override public void messageReceived(LinphoneCore lc, LinphoneChatRoom cr,
      ...
      public void callState(LinphoneCore lc, final LinphoneCall call, LinphoneCall.State state,
        ...
        if (state == State.IncomingReceived) {//来电呼叫
          startIncomingCallActivity();
          return;
        } else if (state == State.Paused
            || state == State.PausedByRemote
            || state == State.Pausing) {//暂停
          if (LinphoneManager.getLc().getCurrentCall() != null) {//禁用视频按钮
            enabledVideoButton(false);
          }
          if (isVideoEnabled(call)) {//视频启用进行通话界面
            showAudioView();
          }
        } else if (state == State.Resuming) {//恢复通话
          if (LinphonePreferences.instance().isVideoEnabled()) {//视频通话
            status.refreshStatusItems(call, isVideoEnabled(call));
            if (call.getCurrentParams().getVideoEnabled()) {
              showVideoView();
            }
          }
          if (LinphoneManager.getLc().getCurrentCall() != null) {//语音通话
            enabledVideoButton(true);
          }
        } else if (state == State.StreamsRunning) {//接通中
        ...
        } else if (state == State.CallUpdatedByRemote) {//当前有视频通话,又来了一个远程视频
          ...
          if (remoteVideo && !localVideo && !autoAcceptCameraPolicy && !LinphoneManager.getLc()
              .isInConference()) {
    ...
    if (findViewById(R.id.fragmentContainer) != null) {
      ...
      if (savedInstanceState != null) {
        ...
        if (savedInstanceState.getBoolean("AskingVideo")) {//是否要求视频
          showAcceptCallUpdateDialog();
          TimeRemind = savedInstanceState.getLong("TimeRemind");
          createTimerForDialog(TimeRemind);
        }
        if (status != null && savedInstanceState.getBoolean("AskingZrtp")) {//要求加密
      ...
      if (isVideoEnabled(LinphoneManager.getLc().getCurrentCall())) {//开启视频
      ...
      if (BluetoothManager.getInstance().isBluetoothHeadsetAvailable()) {//蓝牙耳机使用
  ...
  private void enableAndRefreshInCallActions() {
    ...
    if (LinphoneManager.getLc().isInConference()) {
      confsize =
          LinphoneManager.getLc().getConferenceSize() - (LinphoneManager.getLc().isInConference()
              ? 1 : 0);
    }

    //Enabled transfer button 呼叫没锁定,使用转移按钮
    if (isTransferAllowed && !LinphoneManager.getLc().soundResourcesLocked()) {
      enabledTransferButton(true);
    }

    //Enable conference button 使用会议电话
    ...
    //通话中的设置按钮是否可用
    options.setEnabled(
        !getResources().getBoolean(R.bool.disable_options_in_call) && (addCall.isEnabled()
            || transfer.isEnabled()));

    //如果当前视频可用,媒体设置可用.视频按钮可用
    if (LinphoneManager.getLc().getCurrentCall() != null && LinphonePreferences.instance()
        .isVideoEnabled() && !LinphoneManager.getLc().getCurrentCall().mediaInProgress()) {
      enabledVideoButton(true);
    } else {
      enabledVideoButton(false);
    }
    ...
    //设置暂停按钮状态
    if (LinphoneManager.getLc().getCurrentCall() != null && !LinphoneManager.getLc()
        .getCurrentCall()
        .mediaInProgress()) {
  ...
  @Override public void onClick(View v) {
    ...
    if (id == R.id.video) {//打开视频通话
      int camera =
          getPackageManager().checkPermission(Manifest.permission.CAMERA, getPackageName());
      Log.i("[Permission] Camera permission is " + (camera == PackageManager.PERMISSION_GRANTED
          ? "granted" : "denied"));
    ...
    } else if (id == R.id.micro) {//开启静音模式
      int recordAudio =
          getPackageManager().checkPermission(Manifest.permission.RECORD_AUDIO, getPackageName());
    ...
    } else if (id == R.id.speaker) {//扬声器打开或关闭
      toggleSpeaker();
    } else if (id == R.id.add_call) {//增加新的电话
      goBackToDialer();
    } else if (id == R.id.pause) {//暂停/回复通话
      pauseOrResumeCall(LinphoneManager.getLc().getCurrentCall());
    } else if (id == R.id.hang_up) {//挂断电话
      hangUp();
    } else if (id == R.id.dialer) {//隐藏或显示数字键盘
      hideOrDisplayNumpad();
    } else if (id == R.id.chat) {//打开会话列表
      goToChatList();
    } else if (id == R.id.conference) {//打开会议电话
      enterConference();
      hideOrDisplayCallOptions();
    } else if (id == R.id.switchCamera) {//切换摄像头
      if (videoCallFragment != null) {
        videoCallFragment.switchCamera();
      }
    } else if (id == R.id.transfer) {//返回拨号界面
      goBackToDialerAndDisplayTransferButton();
    } else if (id == R.id.options) {//设置按钮
      hideOrDisplayCallOptions();
    } else if (id == R.id.audio_route) {//音频路由
      hideOrDisplayAudioRoutes();
    } else if (id == R.id.route_bluetooth) {//通话转蓝牙
      ...
      hideOrDisplayAudioRoutes();
    } else if (id == R.id.route_earpiece) {//转成耳机
      ...
      routeEarpiece.setImageResource(R.drawable.route_earpiece_selected);
      hideOrDisplayAudioRoutes();
    } else if (id == R.id.route_speaker) {//转成扬声器
      LinphoneManager.getInstance().routeAudioToSpeaker();
      isSpeakerEnabled = true;
      routeBluetooth.setImageResource(R.drawable.route_bluetooth);
      routeSpeaker.setImageResource(R.drawable.route_speaker_selected);
      routeEarpiece.setImageResource(R.drawable.route_earpiece);
      hideOrDisplayAudioRoutes();
    } else if (id == R.id.call_pause) {//暂停
      LinphoneCall call = (LinphoneCall) v.getTag();
      pauseOrResumeCall(call);
    } else if (id == R.id.conference_pause) {//暂停或回复会议
  ...
  private void disableVideo(final boolean videoDisabled) {
    ...
    if (videoDisabled) {//视频已打开进行更新
      ...
      if (call.getRemoteParams() != null && !call.getRemoteParams().isLowBandwidthEnabled()) {//打开视频
  ...
  private void switchVideo(final boolean displayVideo) {
    ...
    if (!displayVideo) {
      ...
      if (!call.getRemoteParams().isLowBandwidthEnabled()) {//开启视频
        LinphoneManager.getInstance().addVideo();
  ...
  protected void toggleSpeaker() {
    ...
    if (LinphoneManager.getLc().getCurrentCall() != null) {
      if (isVideoEnabled(LinphoneManager.getLc().getCurrentCall()))//视频模式不打开
      {
        LinphoneManager.getInstance().enableProximitySensing(false);
      } else//语音模式打开
      {
  ...
  public void pauseOrResumeCall(LinphoneCall call) {
    LinphoneCore lc = LinphoneManager.getLc();
    if (call != null && LinphoneManager.getLc().getCurrentCall() == call) {//暂停电话
      lc.pauseCall(call);
      if (isVideoEnabled(LinphoneManager.getLc().getCurrentCall())) {//判断是否视频电话
        isVideoCallPaused = true;
      }
      pause.setImageResource(R.drawable.pause_big_over_selected);
    } else if (call != null) {
      if (call.getState() == State.Paused) {//回复通话
        lc.resumeCall(call);
  ...
  private void hangUp() {
    ...
    if (currentCall != null) {//结束当前电话
      lc.terminateCall(currentCall);
    } else if (lc.isInConference()) {//结束会议电话
   ...
   * 设置通话界面用户名/头像等信息
   */
  private void setContactInformation(TextView contactName, ImageView contactPicture,
  ...
  public void refreshCallList(Resources resources) {
    ...
    List<LinphoneCall> pausedCalls =
        LinphoneUtils.getCallsInState(LinphoneManager.getLc(), Arrays.asList(State.PausedByRemote));
   ...
   * 显示化时间
   */
  @SuppressWarnings("deprecation") private void formatText(TextView tv, String name, String value) {
  ...
  private void displayMediaStats(LinphoneCallParams params, LinphoneCallStats stats,
      PayloadType media, View layout, TextView title, TextView codec, TextView dl, TextView ul,
      TextView edl, TextView ice, TextView ip, TextView senderLossRate, TextView receiverLossRate,
      TextView enc, TextView dec, TextView videoResolutionSent, TextView videoResolutionReceived,
      TextView videoFpsSent, TextView videoFpsReceived, boolean isVideo, TextView jitterBuffer) {
    if (stats != null) {
      String mime = null;
      ...
      if (isVideo) {
        formatText(videoResolutionSent, getString(R.string.call_stats_video_resolution_sent),
        ...
        formatText(videoFpsSent, getString(R.string.call_stats_video_fps_sent),
            "\u2191 " + params.getSentFramerate());
        formatText(videoFpsReceived, getString(R.string.call_stats_video_fps_received),
  ...
  public void initCallStatsRefresher(final LinphoneCall call, final View view) {
    ...
    final TextView videoResolutionReceived =
        (TextView) view.findViewById(R.id.video_resolution_received);
            ...
            synchronized (LinphoneManager.getLc()) {
              if (LinphoneActivity.isInstanciated()) {
                ...
                if (params != null) {
                  ...
                  if (params.getVideoEnabled()) videoStats = call.getVideoStats();
                  ...
                  displayMediaStats(params, audioStats, payloadAudio, audioLayout, titleAudio,
                      codecAudio, dlAudio, ulAudio, null, iceAudio, ipAudio, senderLossRateAudio,
                      receiverLossRateAudio, encoderAudio, decoderAudio, null, null, null, null,
                      false, jitterBufferAudio);

                  displayMediaStats(params, videoStats, payloadVideo, videoLayout, titleVideo,
                      codecVideo, dlVideo, ulVideo, edlVideo, iceVideo, ipVideo,
                      senderLossRateVideo, receiverLossRateVideo, encoderVideo, decoderVideo,
                      videoResolutionSent, videoResolutionReceived, videoFpsSent, videoFpsReceived,
                      true, null);
                }
              }
            }
