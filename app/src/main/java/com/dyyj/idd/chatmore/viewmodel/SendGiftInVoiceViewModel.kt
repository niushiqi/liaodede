 ...
 * desc   : 礼物列表
 */
class SendGiftInVoiceViewModel : ViewModel() {
    ...
    //调用送礼物的接口
    fun sendGift(talkId: String, toUserId: String, giftId: String) {
        val subscribe = mDataRepository.sendGiftInVoice(talkId ,toUserId, giftId).subscribe(
                {
                    if (it.errorCode == 200) {
                        //niceToast("礼物已送出~")
