...
import com.dyyj.idd.chatmore.viewmodel.RedPacketPopViewModel
 ...
 * desc   : 领取红包
 */
class RedPacketPop(context: Context?) : BaseTipPop<PopRedPacketBinding, RedPacketPopViewModel>(
    context) {
  override fun onLayoutId(): Int {
    return R.layout.pop_red_packet
  }

  override fun onViewModel(): RedPacketPopViewModel {
    return RedPacketPopViewModel()
  }
  ...
  override fun dismiss() {
    ...
    if (mViewModel.mData != null) {
      EventBus.getDefault().post(RedPacketPopViewModel.GetGiftOkVM(mViewModel.mData!!))
    } else {
      EventBus.getDefault().post(RedPacketPopViewModel.GetGiftOkVM())
    }
  }
   ...
   */
  @Subscribe
  fun onGetGiftVM(obj: RedPacketPopViewModel.GetGiftVM) {
    if (obj.isSuccess) {
