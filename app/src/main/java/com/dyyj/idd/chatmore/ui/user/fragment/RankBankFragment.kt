...
import com.dyyj.idd.chatmore.databinding.FragmentRankingBinding
import com.dyyj.idd.chatmore.model.network.result.BankTopResult
import com.dyyj.idd.chatmore.viewmodel.RankBankViewModel
import com.yqritc.recyclerviewflexibledivider.HorizontalDividerItemDecoration
import org.greenrobot.eventbus.Subscribe

class RankBankFragment : BaseFragment<FragmentRankingBinding, RankBankViewModel>() {
    ...
    override fun onViewModel(): RankBankViewModel {
        return RankBankViewModel()
    }
...
//        mViewModel.getAdapter().initData(list)
        ...
        val head = LayoutInflater.from(context).inflate(R.layout.item_head_bank, null)
    ...
    @Subscribe
    fun OnSubscribBank(vm: RankBankViewModel.RankBankVM) {
        mBinding.swipeLayout.isRefreshing = false
        if (vm.isSuccess) {
//            initRecycleView(vm.obj?.data!!)
            if (vm.more) {
                mViewModel.getAdapter().moreData(vm.obj?.list!!)
            } else {
                initBottom(vm.obj?.userInfo)
                mViewModel.getAdapter().refreshData(vm.obj?.list!!)
            }
        }
    }

    private fun initBottom(userInfo: BankTopResult.BankUserInfo?) {
        mBinding.txtBottomText1.text = userInfo?.nickName
        mBinding.txtBottomText2.text = "银行等级 ${userInfo?.bankLevel}级"
//        mBinding.txtBottomText3.text = "${userInfo?.friendsNum}人"
        mBinding.txtBottomText4.text = "${userInfo?.rank}名"
        mBinding.txtBottomText5.text = "${userInfo?.bankCoin}"
    }
