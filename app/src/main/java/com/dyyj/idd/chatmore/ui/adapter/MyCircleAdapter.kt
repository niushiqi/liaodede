...
class MyCircleAdapter : BaseAdapterV2<UserTopicResult.UserTopic>() {
...
//            mBinding.rvComment.adapter = commentAdapter.value
            ...
            //3大集合打印
            /*Log.i("zhangwj","obj.userInfo?.nickname="+obj.userInfo?.nickname)
            obj.userVoteList!!.forEachIndexed { index, userVote ->
                Log.i("zhangwj","index"+index+"obj.userVote?.nickname="+userVote.nickname+"obj.userVote?.replyVirtualName="+userVote?.replyVirtualName)
            }
            obj.userReplyList!!.forEachIndexed { index, userReply ->
                Log.i("zhangwj","index"+index+"obj.userReply?.nickname="+userReply.nickname+"obj.userReply?.replyVirtualName="+userReply.replyVirtualName)
            }*/
             ...
             * 赞列表
             */
            if (obj.userId!!.equals(mDataRepository.getUserid()!!)) {//如果是自己的动态
                    ...
                    obj.userReplyList?.subList(0, obj.showNums)?.let {
                        obj.userReplyList.subList(0, obj.showNums).forEachIndexed { index, userReply ->
                            if (obj.userId.equals(mDataRepository.getUserid())) {
                                insertTextView(userReply,true)
                            }else{
                                insertTextView(userReply,false)
                            }
                        }
                    }
                } else {
...
//                    obj.userReplyList?.let { commentAdapter.value.refreshData(obj.userReplyList) }
            ...
            mBinding.txtMore.setOnClickListener {
                if (obj.topicReplyCount?.toInt() ?: 0 > 0) {
                    obj.showNums = obj.topicReplyCount?.toInt()!!//点击更多,展示全部
                    //Log.i("zhangwj","obj.showNums="+obj.showNums)
                }
            ...
            mBinding.ivComment.setOnClickListener {
                ...
                obj.Id?.let {
                    EventBus.getDefault().post(MyCircleViewModel.MyCommentSendVM(obj.Id,obj.userId?:""))
...
//                                    if (it.errorCode == 200) {
//                                        obj.agree = 0
//                                        obj.topicVoteCount = (obj.topicVoteCount?.toInt()?:1 - 1).toString()
//                                        mBinding.ivLike.setImageResource(R.drawable.ic_dynamic_like)
//                                        mBinding.txtLike.text = "+${obj.topicVoteCount}"
//                                        mBinding.txtLike.setTextColor(Color.parseColor("#7F8D9C"))
//                                    } else {
...
//                } else {
                    obj.Id?.let {
                        mDataRepository.voteTopic(mDataRepository.getUserid()!!, obj.Id).subscribe {
                            if (it.errorCode == 200) {
                                obj.isVote = 1
                                obj.topicVoteCount = (1 + (obj.topicVoteCount?.toInt()
                                        ?: 0)).toString()
                                mBinding.ivLike.setImageResource(R.drawable.ic_dynamic_like_ok)
                                mBinding.txtLike.text = "+${obj.topicVoteCount}"
                                mBinding.txtLike.setTextColor(Color.parseColor("#FF715F"))
                                mBinding.llLikes.visibility = View.VISIBLE
//                                if (mBinding.txtLikes.text.isNotBlank()) {
//                                    mBinding.txtLikes.text = mBinding.txtLikes.text.toString() + "、" + mDataRepository.getUserInfoEntity()?.nickname
//                                } else {
//                                    mBinding.txtLikes.text = mDataRepository.getUserInfoEntity()?.nickname
//                                }
