...
import com.dyyj.idd.chatmore.databinding.FragmentRankPopBinding
import com.dyyj.idd.chatmore.model.network.result.PopularityTopResult
import com.dyyj.idd.chatmore.viewmodel.RankPopularViewModel
import com.yqritc.recyclerviewflexibledivider.HorizontalDividerItemDecoration
import org.greenrobot.eventbus.Subscribe

class RankPopularFragment: BaseFragment<FragmentRankPopBinding, RankPopularViewModel>() {
    ...
    override fun onLayoutId(): Int {
        return R.layout.fragment_rank_pop
    }

    override fun onViewModel(): RankPopularViewModel {
        return RankPopularViewModel()
    }
...
//        mViewModel.getAdapter().initData(list)
        ...
        val head = LayoutInflater.from(context).inflate(R.layout.item_head_popular, null)
    ...
    @Subscribe
    fun OnSubscribBank(vm: RankPopularViewModel.RankPopularVM) {
...
//            initRecycleView(vm.obj?.data!!)
    ...
    private fun initBottom(userInfo: PopularityTopResult.PopularityUserInfo?) {
        mBinding.txtBottomText1.text = userInfo?.nickName
        mBinding.txtBottomText2.text = "${userInfo?.rank}"
        mBinding.txtBottomText3.text = "${userInfo?.friendsNum}"
    }
